...
function build_for_linux() {
  ...
  docker run -it --rm -v="$GOPATH:/go" $container sh -c \
    "cd /go/src/github.com/soracom/endorse-client-go/cmd/endorse-cli && \
...
: "Build endorse-cli executables" && {
    pushd "$d/cmd/endorse-cli" > /dev/null
    echo "Building artifacts ..."
    rm -rf "$d/cmd/endorse-cli/dist/"
